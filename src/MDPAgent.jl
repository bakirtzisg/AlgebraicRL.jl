...
# This is the main function of this object. Given that it contains an MDP following the interface, and an already trained Agent,
# it simulates a single episode following the policy of the agent.
# it returns the reward of tha episode
function solve(mdpAgent::MDPAgent)
...
# overwrite base.collect for our class since it will be used by Algebraic dynamics. Simply return the mdpAgent since there is nothing to collect.
# this will be used for discrete machines 
...
# The only thing i did was remove the reset!(env) because we already do that before calling solve. See MDPAgentMAchine. 
